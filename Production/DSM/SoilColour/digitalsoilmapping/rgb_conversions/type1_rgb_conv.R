...
root.slurm<- "/datasets/work/af-tern-mal-deb/work/projects/ternlandscapes_2019/soilColour/rcode/slurm/spatialprediction/rgb"
...
oper1<- foreach(i=srt:fin, .packages = c("raster", "sp", "rgdal")) %dopar% {
  ...
  tempD$type1_surface_R<- NA
  tempD$type1_surface_G<- NA
  tempD$type1_surface_B<- NA
  tempD$type1_subsoil_R<- NA
  tempD$type1_subsoil_G<- NA
  tempD$type1_subsoil_B<- NA
  tempD$type1_surface_cieL<- NA
  tempD$type1_surface_cieA<- NA
  tempD$type1_surface_cieB<- NA
  tempD$type1_subsoil_cieL<- NA
  tempD$type1_subsoil_cieA<- NA
  tempD$type1_subsoil_cieB<- NA
  tempE <- tempD[comca, ]
  ...
  for (j in 1:nrow(tempE)){
    ...
    # disitance calcs
    d1<-sqrt((tempE$surface_model_1_L[j] -  ref.colors$L.)^2 + (tempE$surface_model_5_A[j] -  ref.colors$a.)^2 + (tempE$surface_model_9_B[j] -  ref.colors$b.)^2)
    d1.sel<- which(d1 == min(d1))
    d2<-sqrt((tempE$subsoil_model_1_L[j] -  ref.colors$L.)^2 + (tempE$subsoil_model_5_A[j] -  ref.colors$a.)^2 + (tempE$subsoil_model_9_B[j] -  ref.colors$b.)^2)
