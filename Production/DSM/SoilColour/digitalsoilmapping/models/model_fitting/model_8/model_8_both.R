...
capture.root<- "/datasets/work/af-tern-mal-deb/work/projects/ternlandscapes_2019/soilColour/models/model_fitting/model_8/"
var_nm1<- paste0(capture.root,"rangerModel_params_surface_model_8.txt")
var_nm2<- paste0(capture.root,"rangerModel_params_subsoil_model_8.txt")
mod.out.sur<- paste0(capture.root,"surface_model_8.rds")
mod.out.sub<- paste0(capture.root,"subsoil_model_8.rds")
...
names(top.data)
ranger.model1<-train(x= top.data[,c(9,11,12:50)], y= top.data$A, 
                     tuneGrid = tgrid,
                     method = "ranger",
...
names(sub.data)
ranger.model1<-train(x= sub.data[,c(9,11,12:50)], y= sub.data$A, 
                     tuneGrid = tgrid,
                     method = "ranger",
                     trControl =trainControl(method = "oob"), 
                     num.trees = 500, 
