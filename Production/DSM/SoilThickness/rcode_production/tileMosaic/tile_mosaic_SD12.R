## SOIL DEPTH
# Probability of deep soil (>2m) not to occur
...
for (i in 1:length(fols)){
  ...
  r1<- raster(list.files(fpath1, pattern="not_deep_prob_SD.tif", full.names=TRUE))
  raster_list <- append(raster_list, r1)
  print(i)
}

#raster_list
raster_list$filename <- "/OSM/CBR/AF_TERN_MAL_DEB/WORK/projects/soilDepth_2019/soilDepth/spatialPrediction/not_deep_prob_SD.tif"
