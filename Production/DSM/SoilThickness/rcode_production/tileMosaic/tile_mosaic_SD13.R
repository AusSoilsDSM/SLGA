## SOIL DEPTH
# Probability of surface being a rock outcrop
...
for (i in 1:length(fols)){
  ...
  r1<- raster(list.files(fpath1, pattern="rockoutcrop_prob_SD.tif", full.names=TRUE)[2])
  raster_list <- append(raster_list, r1)
  print(i)
}

#raster_list
raster_list$filename <- "/OSM/CBR/AF_TERN_MAL_DEB/WORK/projects/soilDepth_2019/soilDepth/spatialPrediction/rockoutcrop_prob_SD.tif"
