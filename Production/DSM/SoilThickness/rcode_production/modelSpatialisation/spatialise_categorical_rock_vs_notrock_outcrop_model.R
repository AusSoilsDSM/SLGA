### soil depth 2019
## spatial prediction of categorical models (rock outcrop prediction)
## modified: 26.7.2019
...
models<- list.files(path = "/OSM/CBR/AF_TERN_MAL_DEB/WORK/projects/soilDepth_2019/soilDepth/models/", pattern = "aus_soilDepth_binary_GAR_model_batch_",full.names = TRUE)
...
oper1<- foreach(i=1:length(fols), .packages = c("raster", "sp", "rgdal", "caret", "ranger")) %dopar% {
  ...
  for (z in 1:50){  
    ...
    nm4<- dir.list[z+1]
    nm5<- paste(nm4, "/", list.dirs(nm3,full.names = F)[z+1], "_soilDepth__GAR_cat_pred.tif", sep="")
  ...
  nmz<- paste0("/OSM/CBR/AF_TERN_MAL_DEB/WORK/projects/soilDepth_2019/soilDepth/rcode/slurm/slurmCheck/GAR1/b1/Iteration_counter_", batch, "_", i, ".txt")
  write.table(itOuts, 
              file = nmz,
