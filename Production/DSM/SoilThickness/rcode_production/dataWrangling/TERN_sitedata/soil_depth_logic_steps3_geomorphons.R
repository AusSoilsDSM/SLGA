...
# load in site observation data from step 2 which has site obs with associated estimates of maximum soil depth and
# whether or not the data is censored or not.

# We want to determine whether there is some relationship between landform and soil depth
# also check whether land curvature can help determine soil depth
...
# At this stage just work withthe non censored data ie those sites where we know we have reached lithic contact
#nc_dat<- dat[dat$censored==0,]
#nrow(nc_dat)/nrow(dat)



## raster of the geomorphons
r1<- raster("Y:/CoVariates/Relief/relief_geomorphons.tif")
r2<- raster("Y:/CoVariates/Relief/relief_plan_curvature_3s.tif")
r3<- raster("Y:/CoVariates/Relief/relief_profile_curvature_3.tif")
#reproject landsat grid
r1_pr <- projectRaster(from =r1, to = r2 ,method="ngb")
s1<- stack(r1_pr,r2,r3)
crs(s1)

#intersect points with raster
names(dat)
coordinates(dat)<- ~ o_longitude_GDA94 + o_latitude_GDA94
crs(dat)<- "+proj=longlat +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +no_defs" #GDA94
nc_dat1<- spTransform(dat, crs(s1))
...
nc_dat2<- extract(s1, nc_dat1, method="simple", sp=1)
nc_dat2<- as.data.frame(nc_dat2)

#filter out missing data
names(nc_dat2)
nc_dat3<- nc_dat2[complete.cases(nc_dat2[,12:14]),]

write.table(nc_dat3,paste0("C:/Users/", ident, "/Dropbox/2018/soilDepth/data/dbo_Horzons_loc_sub_step3_Out1.txt"),row.names = F,col.names = T,sep=",")
