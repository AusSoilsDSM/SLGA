...
data_boreLith<- readRDS("Y:/DepthData/data_boreLith.rds")
data_boreLoc1<- readRDS("Y:/DepthData/data_boreLoc1.rds")
data_boreLoc2<- readRDS("Y:/DepthData/data_boreLoc2.rds")
...
for (i in 208368:length(levels(data_boreLith$HydroCode))){ # for each level of the lithology hydrocode data
  ...
  lith_dat_sp<- split(lith_datT, lith_datT$BoreID)
  length(lith_dat_sp)
