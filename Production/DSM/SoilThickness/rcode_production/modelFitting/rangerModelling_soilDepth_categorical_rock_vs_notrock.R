...
## Modelling presence of rock outcrops
### modified 23/07/19
...
for (i in 1:50){
  ...
  #fit model
  ranger.model<-train(x= DSM_data_c[,c(6, 8:42)], y= DSM_data_c$GA_rocks,
                       tuneGrid = tgrid, 
                       method = "ranger",
  ...
  ## capture output
  var_nm1<- paste0("/OSM/CBR/AF_TERN_MAL_DEB/WORK/projects/soilDepth_2019/soilDepth/models/","aus_soilDepth_binary_GAR_model_variableImportance_batch_", cnt, "_iteration_", i, ".txt")
  ...
  # save model to file
  mod_name<- paste0("/OSM/CBR/AF_TERN_MAL_DEB/WORK/projects/soilDepth_2019/soilDepth/models/","aus_soilDepth_binary_GAR_model_batch_", cnt, "_iteration_", i, ".rds")
  ...
  # goof_cat function
  gc<- goof_cat(observed = DSM_data_c$GA_rocks, predicted = ranger.pred_c)
  ...
  val_diog[i, 1]<- cnt
  val_diog[i, 2]<- i
  gv<- goof_cat(observed = DSM_data_v$GA_rocks, predicted = ranger.pred_v)
  ...
  cal_name<- paste0("/OSM/CBR/AF_TERN_MAL_DEB/WORK/projects/soilDepth_2019/soilDepth/models/","binary_GAR_model_CAL_diogs_model_batch_", cnt, ".txt")
  ...
  val_name<- paste0("/OSM/CBR/AF_TERN_MAL_DEB/WORK/projects/soilDepth_2019/soilDepth/models/","binary_GAR_model_VAL_diogs_model_batch_", cnt, ".txt")
  write.table(val_frame,file = val_name, sep = ",",row.names = F, col.names = T)
    
    
  #output observation and prediction for calibration sites
  calOut_dat<- cbind(DSM_data_c[,c(1,2,3,7,43:45)],ranger.pred_c)
  calOut_dat$prediction<- ranger.pred_c
  cal_name1<- paste0("/OSM/CBR/AF_TERN_MAL_DEB/WORK/projects/soilDepth_2019/soilDepth/models/","GAR_model_CAL_preds_batch_", cnt, "_iteration_", i, ".txt")
  write.table(calOut_dat,file = cal_name1, sep = ",",row.names = F, col.names = T) 
    
  #output observation and prediction for validation sites
  valOut_dat<- cbind(DSM_data_v[,c(1,2,3,7,43:45)],ranger.pred_v)
  valOut_dat$prediction<- ranger.pred_v
  val_name1<- paste0("/OSM/CBR/AF_TERN_MAL_DEB/WORK/projects/soilDepth_2019/soilDepth/models/","GAR_model_VAL_preds_batch_", cnt, "_iteration_", i, ".txt")
