...
input.data<- read.csv(file = paste0(input.root,"ranger_EXT_preds_totalP_alldepths_summary.csv"))
names(input.data)
ext.covariate.dat<- readRDS(paste0(data.root,"tern_totalP_labelled_covariates_TEST.rds"))
...
# covariance matrix
cov.matrix<- readRDS(file = paste0(data.root,"tern_totalP_covariates_varcov_matrix_CAL.rds"))
...
for (i in 1:length(cluster.files)){
  ...
  write.csv(x = summary.mat.exp, file = paste0(data.outs, "_temp_summary_stats_uncertainty_optimisation_alldepths.csv"),row.names = F)
...
write.csv(x = summary.mat, file = paste0(data.outs, "summary_stats_uncertainty_optimisation_alldepths.csv"),row.names = F)
