...
for (xx in 1:6){
  ...
  # Point data
  vart<- "totalP"
  test<- read.csv(file = paste0(data.root,"ranger_EXT_preds_",vart,"_alldepths_summary.csv"))
  
  # subset data
  DSM_data<- test[test$depth == depth, ]
  names(DSM_data)
  ...
  xlimits= c(0,1.5)
  ylimits= c(0,1.5) 
  tiff(file=paste0(fig.root,"external_val_pred_d",depth,".tiff"),width=12,height=12,units="cm",res=300,pointsize=8)
  ...
  write.csv(x = ext.pred.goof.frame, file = paste0(fig.root,"external_val_pred_d",depth,".csv"), row.names = F)
}
# END
