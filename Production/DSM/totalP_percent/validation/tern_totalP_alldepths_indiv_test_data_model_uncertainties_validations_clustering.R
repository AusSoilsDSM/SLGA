...
for (xx in 1:6){
  ...
  # load data
  input.data<- read.csv(file = paste0(input.root,"ranger_EXT_preds_",vart,"_alldepths_summary.csv"))
  names(input.data)
  ...
  saveRDS(object = weighted.outs.mat, file = paste0(input.root,"clustering_EXT_preds_",vart, "_depth_d",depth,"_upper_and_lower.rds"))
  
  picp.outs.mat<- as.data.frame(picp.outs.mat)
  names(picp.outs.mat)<- probs.check
  saveRDS(object = picp.outs.mat, file = paste0(input.root,"clustering_EXT_preds_", vart, "depth_d",depth,"_picp.rds"))
  
  range.outs.mat<- as.data.frame(range.outs.mat)
  names(range.outs.mat)<- probs.check
  saveRDS(object = range.outs.mat, file = paste0(input.root,"clustering_EXT_preds_",vart, "_depth_d",depth,"_predictionrange.rds"))
  ...
  saveRDS(object = output.dat, file = paste0(input.root,"clustering_EXT_preds_",vart, "_depth_",depth,"_summary_w_upper_and_lower.rds"))
  write.csv(x =  output.dat, file = paste0(input.root,"clustering_EXT_preds_", vart, "_depth_d",depth,"_summary_w_upper_and_lower.csv"),row.names=F)
  ...
  xlimits= c(0,100)
  ylimits= c(0,100) 
  tiff(file=paste0(data.outs,"external_val_", vart, "_PICP_clustering_d",depth,".tiff"),width=12,height=12,units="cm",res=300,pointsize=8)
