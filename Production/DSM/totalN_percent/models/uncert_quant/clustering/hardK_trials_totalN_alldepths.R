...
#data
# site data
site.dat<- readRDS(paste0(data.root,"tern_totalN_labelled_covariates_CAL.rds"))
names(site.dat)
...
# save matrix
saveRDS(object = varcov.mat, file = paste0(data.root,"tern_totalN_covariates_varcov_matrix_CAL.rds"))
...
## join up with labels
names(site.dat)
label.dat<- site.dat[,1:6]
out.dat<- cbind(label.dat,cluster.matrix)

# save data
saveRDS(object = out.dat, file = paste0(cluster.out,"cluster_centers_totalN_all.rds"))
