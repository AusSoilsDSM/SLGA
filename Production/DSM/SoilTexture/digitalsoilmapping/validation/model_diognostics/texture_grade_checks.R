...
## DEPTH 1
### clay external observations and predictions: depth 1
vart<- "clay"
depth<- "d1"
...
names(clay.dat)
clay.quants<- rowQuantiles(as.matrix(clay.dat[,7:56]), probs=c(0.05, 0.5, 0.95))

### sand external observations and predictions: depth 1
vart<- "sand"
depth<- "d1"
...
names(sand.dat)
sand.quants<- rowQuantiles(as.matrix(sand.dat[,7:56]), probs=c(0.05, 0.5, 0.95))
...
for (g in 1:10){
  ...
  for (j in 1:nrow(clay.dat)){
    ...
    for (z in 7:56){
      ...
      dist.sum<-  sum(as.numeric(order(pred.dist)[1:5] %in% order(obs.dist)[1:5]))
