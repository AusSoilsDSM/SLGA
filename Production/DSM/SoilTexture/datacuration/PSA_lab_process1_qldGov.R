...
G1<- c("X2Z2_Clay", "X2Z2_CS","X2Z2_FS","X2Z2_TotSan","X2Z2_Silt")
G2<- c("P10_CF_C","P10_CF_CS","P10_CF_FS","P10_CF_S","P10_CF_Z")
G3<- c("P10_NR_C","P10_NR_CS","P10_NR_FS","P10_NR_S","P10_NR_Z")
G4<- c("P10_PB_C","P10_PB_CS","P10_PB_FS","P10_PB_S","P10_PB_Z")
...
proc_1$X2Z2_Clay<- as.numeric(as.character(proc_1$X2Z2_Clay))
proc_1$X2Z2_CS<- as.numeric(as.character(proc_1$X2Z2_CS))
proc_1$X2Z2_FS<- as.numeric(as.character(proc_1$X2Z2_FS))
proc_1$X2Z2_TotSan<- as.numeric(as.character(proc_1$X2Z2_TotSan))
proc_1$X2Z2_Silt<- as.numeric(as.character(proc_1$X2Z2_Silt))
...
for (i in 1:length(levs)){
  ...
  for (j in 1:nrow(subs)){
    ...
    if(vals$chvec == 0){
      subs[j,"clay"]<- vals$X2Z2_Clay
      subs[j,"silt"]<- vals$X2Z2_Silt
      subs[j,"sand"]<- vals$X2Z2_TotSan 
    ...
    if(vals$chvec == 1 & is.na(vals$X2Z2_TotSan)){
      subs[j,"clay"]<- vals$X2Z2_Clay
      subs[j,"silt"]<- vals$X2Z2_Silt
      subs[j,"sand"]<- vals$X2Z2_CS + vals$X2Z2_FS
      sval<- sum(subs[j,11:13])
      # check to see if numbers make sense
      if(sval < 90) {
        subs[j, "com"]<- -9999} else {
    ...
    if(vals$chvec == 2 & !is.na(vals$X2Z2_Clay) & !is.na(vals$X2Z2_TotSan) & !is.na(vals$X2Z2_Silt) ){
      subs[j,"clay"]<- vals$X2Z2_Clay
      subs[j,"silt"]<- vals$X2Z2_Silt
      subs[j,"sand"]<- vals$X2Z2_TotSan 
      sval<- sum(subs[j,11:13])
    ...
    if(vals$chvec == 3 & !is.na(vals$X2Z2_Clay) & !is.na(vals$X2Z2_TotSan) & is.na(vals$X2Z2_Silt) ){
      subs[j,"clay"]<- vals$X2Z2_Clay
      subs[j,"silt"]<- 100 - vals$X2Z2_Clay - vals$X2Z2_TotSan
      subs[j,"sand"]<- vals$X2Z2_TotSan  
      sval<- sum(subs[j,11:13])
      # check to see if numbers make sense
      if(sval < 90) {
    ...
    if(vals$chvec == 3 & !is.na(vals$X2Z2_Clay) & is.na(vals$X2Z2_TotSan) & !is.na(vals$X2Z2_Silt) ){
      subs[j,"clay"]<- vals$X2Z2_Clay
      subs[j,"silt"]<- vals$X2Z2_Silt
      subs[j,"sand"]<- 100 -  vals$X2Z2_Clay - vals$X2Z2_Silt
      sval<- sum(subs[j,11:13])
      # check to see if numbers make sense
      if(sval < 90) {
        subs[j, "com"]<- -9999} else {
...
for (i in 1:length(levs)){
  ...
  for (j in 1:nrow(subs)){
    ...
    # if only no values are missing
    if(vals$chvec == 0){
      subs[j,"clay"]<- vals$P10_CF_C
      subs[j,"silt"]<- vals$P10_CF_Z
      subs[j,"sand"]<- vals$P10_CF_S 
      sval<- sum(subs[j,11:13])
      # check to see if numbers make sense
      if(sval < 90) {
        subs[j, "com"]<- -9999} else {
    ...
    # if only one value is missing and it is sand 
    if(vals$chvec == 1 & is.na(vals$P10_CF_S)){
      subs[j,"clay"]<- vals$P10_CF_C
      subs[j,"silt"]<- vals$P10_CF_Z
      subs[j,"sand"]<- vals$P10_CF_CS + vals$P10_CF_FS
      sval<- sum(subs[j,11:13])
      # check to see if numbers make sense
      if(sval < 90) {
        subs[j, "com"]<- -9999} else {
    ...
    # if we have data about all three fractions [clay, sand, silt]
    if(vals$chvec == 2 & !is.na(vals$P10_CF_C) & !is.na(vals$P10_CF_S) & !is.na(vals$P10_CF_Z) ){
      subs[j,"clay"]<- vals$P10_CF_C
      subs[j,"silt"]<- vals$P10_CF_Z
      subs[j,"sand"]<- vals$P10_CF_S 
      sval<- sum(subs[j,11:13])
    ...
    # if we have data about clay and sand fraction but not silt
    if(vals$chvec == 3 & !is.na(vals$P10_CF_C) & !is.na(vals$P10_CF_S) & is.na(vals$P10_CF_Z) ){
      subs[j,"clay"]<- vals$P10_CF_C
      subs[j,"silt"]<- 100 - vals$P10_CF_C - vals$P10_CF_S
      subs[j,"sand"]<- vals$P10_CF_S  
      sval<- sum(subs[j,11:13])
      # check to see if numbers make sense
      if(sval < 90) {
    ...
    if(vals$chvec == 3 & !is.na(vals$P10_CF_C) & is.na(vals$P10_CF_S) & !is.na(vals$P10_CF_Z) ){
      subs[j,"clay"]<- vals$P10_CF_C
      subs[j,"silt"]<- vals$P10_CF_Z
      subs[j,"sand"]<- 100 -  vals$P10_CF_C - vals$P10_CF_Z
      sval<- sum(subs[j,11:13])
      # check to see if numbers make sense
      if(sval < 90) {
        subs[j, "com"]<- -9999} else {
...
#test2<- proc_1[which(proc_1$com == -9999),]
#proc_1<- proc_1[-which(proc_1$com == -9999),] 
proc_z<- rbind(proc_z, proc_1[,c(1:13,21)])
...
# save output
saveRDS(nproc_z, file = "Z:/projects/ternlandscapes_2019/soiltexture/data/process1/QLDgov_lab_PSA_data.rds")
