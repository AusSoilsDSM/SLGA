...
G2<- c("P10_HYD_C","P10_HYD_CS","P10_HYD_FS","P10_HYD_S","P10_HYD_Z")
...
for (i in 1:length(levs)){
  ...
  for (j in 1:nrow(subs)){
    ...
    # if only no values are missing
    if(vals$chvec == 0){
      subs[j,"clay"]<- vals$P10_HYD_C
      subs[j,"silt"]<- vals$P10_HYD_Z
    ...
    if(vals$chvec == 1 & is.na(vals$P10_HYD_S)){
      subs[j,"clay"]<- vals$P10_HYD_C
      subs[j,"silt"]<- vals$P10_HYD_Z
      subs[j,"sand"]<- vals$P10_HYD_CS + vals$P10_HYD_FS
      sval<- sum(subs[j,11:13])
      # check to see if numbers make sense
      if(sval < 90) {
        subs[j, "com"]<- -9999} else {
    ...
    # if we have data about all three fractions [clay, sand, silt]
    if(vals$chvec == 2 & !is.na(vals$P10_HYD_C) & !is.na(vals$P10_HYD_S) & !is.na(vals$P10_HYD_Z) ){
      subs[j,"clay"]<- vals$P10_HYD_C
      subs[j,"silt"]<- vals$P10_HYD_Z
      subs[j,"sand"]<- vals$P10_HYD_S 
      sval<- sum(subs[j,11:13])
    ...
    # if we have data about clay and sand fraction but not silt
    if(vals$chvec == 3 & !is.na(vals$P10_HYD_C) & !is.na(vals$P10_HYD_S) & is.na(vals$P10_HYD_Z) ){
      subs[j,"clay"]<- vals$P10_HYD_C
      subs[j,"silt"]<- 100 - vals$P10_HYD_C - vals$P10_HYD_S
      subs[j,"sand"]<- vals$P10_HYD_S  
      sval<- sum(subs[j,11:13])
      # check to see if numbers make sense
      if(sval < 90) {
    ...
    if(vals$chvec == 3 & !is.na(vals$P10_HYD_C) & is.na(vals$P10_HYD_S) & !is.na(vals$P10_HYD_Z) ){
      subs[j,"clay"]<- vals$P10_HYD_C
      subs[j,"silt"]<- vals$P10_HYD_Z
      subs[j,"sand"]<- 100 -  vals$P10_HYD_C - vals$P10_HYD_Z
      sval<- sum(subs[j,11:13])
      # check to see if numbers make sense
      if(sval < 90) {
        subs[j, "com"]<- -9999} else {
...
# save output
saveRDS(nproc_z, file = "Z:/projects/ternlandscapes_2019/soiltexture/data/process1/natsoil_lab_PSA_data.rds")
