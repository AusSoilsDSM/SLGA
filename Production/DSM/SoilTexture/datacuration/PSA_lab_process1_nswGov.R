...
G1<- c("X2Z2_Clay_N", "X2Z2_Silt_N","X2Z2_TotSan")
...
for (j in 1:nrow(proc_1)){
  proc_1$chvec[j]<- sum(as.numeric(is.na(proc_1[j,14:16])))}

# remove instances of all missing values along vector
proc_1<- proc_1[proc_1$chvec != 3,]
proc_1$com<- NA
proc_1$methgroup<- NA
...
for (i in 1:length(levs)){
  ...
  for (j in 1:nrow(subs)){
    ...
    if(vals$chvec == 1 & !is.na(vals$X2Z2_Clay_N) & !is.na(vals$X2Z2_Silt_N) ){
      subs[j,"clay"]<- vals$X2Z2_Clay_N
      subs[j,"silt"]<- vals$X2Z2_Silt_N
      subs[j,"sand"]<- 100 - vals$X2Z2_Clay_N - vals$X2Z2_Silt_N
...
# save output
saveRDS(nproc_z, file = "Z:/projects/ternlandscapes_2019/soiltexture/data/process1/NSWgov_lab_PSA_data.rds")
