...
## Prep data prior to covariate data intersection (morphological data)
...
## morphological data
morph.files<- list.files(path = "Z:/projects/ternlandscapes_2019/soiltexture/data/process4/morph_sims",full.names = T)
...
names(morph.dat1)
ids.cols<- morph.dat1[,17:18]

morph.files


for (i in 1:length(morph.files)){
  dat<- readRDS(morph.files[i])
  dat<- cbind(ids.cols,dat)
  saveRDS(object = dat, file = morph.files[i])
  print(i)}
...
for (i in 1:length(morph.files)){
  ...
  yave.name<- paste0("Z:/projects/ternlandscapes_2019/soiltexture/data/","splined_morph_comp_clay_sim_",i, ".rds")
  saveRDS(yave,file = yave.name)
  
  # Error checking between obs and spline predictions
  dave.name<- paste0("Z:/projects/ternlandscapes_2019/soiltexture/data/","splined_morph_comp_clay_errors_sim_",i, ".rds")
  saveRDS(dave,file = dave.name)
  
  # Original dataset but flagged for data to remove
  ddat.name<- paste0("Z:/projects/ternlandscapes_2019/soiltexture/data/","splined_morph_comp_clay_origdat_sim_",i, ".rds")
  ...
  # Export data 
  # Values for the specified depths
  yave.name<- paste0("Z:/projects/ternlandscapes_2019/soiltexture/data/","splined_morph_comp_sand_sim_",i, ".rds")
  saveRDS(yave,file = yave.name)
  
  # Error checking between obs and spline predictions
  dave.name<- paste0("Z:/projects/ternlandscapes_2019/soiltexture/data/","splined_morph_comp_sand_errors_sim_",i, ".rds")
  saveRDS(dave,file = dave.name)
  
  # Original dataset but flagged for data to remove
  ddat.name<- paste0("Z:/projects/ternlandscapes_2019/soiltexture/data/","splined_morph_comp_sand_origdat_sim_",i, ".rds")
  saveRDS(d.dat,file = ddat.name)
  ...
  lab.dat$Observation_ID<- as.character(lab.dat$Observation_ID)
  unique.ids<- unique(lab.dat$Observation_ID)
  clay.dat<- readRDS(file = paste0("Z:/projects/ternlandscapes_2019/soiltexture/data/","splined_morph_comp_clay_sim_",i, ".rds"))
  sand.dat<- readRDS(file = paste0("Z:/projects/ternlandscapes_2019/soiltexture/data/","splined_morph_comp_sand_sim_",i, ".rds"))
  names(lab.dat)
  ...
  saveRDS(lab.dat,file = paste0("Z:/projects/ternlandscapes_2019/soiltexture/data/","morph_psa_data_splined_dsm_extract_prepared_sim_",i, ".rds"))
  }
