...
## Lab data
## Step 3: splining of data

root<- "Z:/projects/ternlandscapes_2019/soil_pH/data/curated_step2"
root.out<- "Z:/projects/ternlandscapes_2019/soil_pH/data/curated_step3/"


files<- list.files(path = root, pattern = ".rds",full.names = T)
files
files.short<- list.files(path = root, pattern = ".rds",full.names = F)
...
# Go through each dataset
  lab.dat<- readRDS(files[6])
  write.csv(lab.dat, file = paste0(root,"/",files.short[6],".csv"), row.names = F)
...
names(lab.dat)
d.dat<- as.data.frame(lab.dat[,c(21,9,10,13,12)])
...
write.csv(yave, paste0(root.out,"splined_lab_meth6.csv"),row.names = F)
saveRDS(yave,paste0(root.out,"splined_lab_meth6.rds"))

# Error checking between obs and spline predictions
write.csv(dave, paste0(root.out,"splined_lab_meth6_errors.csv"), row.names = F)
saveRDS(dave,paste0(root.out,"splined_lab_meth6_errors.rds"))

# Original dataset but flagged for data to remove
write.csv(d.dat, paste0(root.out,"splined_lab_meth6_origdat.csv"),row.names = F)
saveRDS(d.dat,paste0(root.out,"splined_lab_meth6_origdat.rds"))
...
for (i in 1:length(unID)){
  subs<- lab.dat[lab.dat$nloc_fid == unID[i],]
  subz<- subs[1,]
  # add info
  yave$Dataset[i]<- subz$Dataset
  yave$Observation_ID[i]<- subz$Observation_ID
  yave$Longitude[i]<- subz$Longitude
  yave$Latitude[i]<- subz$Latitude
  yave$ObservedProperty[i]<- subz$ObservedProperty
  yave$nloc_fid[i]<- subz$nloc_fid
  yave$SampleDate[i]<- subz$SampleDate
  
}

names(yave)
yave<- yave[,c(14,9:13,15,1:7)]

# Original dataset but flagged for data to remove
write.csv(yave, paste0(root.out, "lab_ph_4A1_data_splined_dsm_extract_prepared.csv"),row.names = F)
saveRDS(yave,paste0(root.out, "lab_ph_4A1_data_splined_dsm_extract_prepared.rds"))
