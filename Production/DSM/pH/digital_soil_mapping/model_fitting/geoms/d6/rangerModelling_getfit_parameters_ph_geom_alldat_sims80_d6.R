...
hist(log(sub.site.dat$target))
sub.site.dat$transformed<- log(1- sub.site.dat$target)
summary(sub.site.dat$transformed)
names(sub.site.dat)

ranger.model<-train(x= sub.site.dat[,2:40], y= sub.site.dat$target,method = "ranger",trControl = trainControl(method = "oob"), num.trees =  500)  
summary(ranger.model)
ranger.model

## capture output
var_nm1<- paste0(params.out,"rangerModel_alldat_sims80_params_",vart, "_",depth,".txt")
var_nm1
