...
# Depth 6 [100-200cm]
# Code involves the quant of field data to a 'lab' estimate base don the empirical relations between field and lab measures
...
# lab dat
names(site.dat)
sub.site.dat<- site.dat[,c(1:8,14,16,17:55)] # change column selection for target variable
sub.site.dat<- sub.site.dat[complete.cases(sub.site.dat[,c(9,11:49)]),]
sub.site.dat$target<- sub.site.dat[,9]
...
## capture output
var_nm1<- paste0(params.out,"rangerModel_params_ph4a1_d6.txt")
...
saveRDS(ranger.model, paste0(model.out,"rangerModel_params_ph4a1_d6.rds"))
