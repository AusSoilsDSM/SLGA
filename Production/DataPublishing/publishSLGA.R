...
att = args[2]
dataDir <- args[3]
adjustVal <- as.numeric(args[4])
fillHoles <- as.logical(args[5])
...
op <- publishRaster(inRasterPath=inRasterPath, outDir=outDir, resampleTemplate=templateR, adjustVal=adjustVal, fillHoles=fillHoles, sigFigures=sigFigures, maskTemplate=templateR, metadataTable=mt, RATTable=NULL, MakeCOG=makeCOG)
