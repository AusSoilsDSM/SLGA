...
data.in<- paste0(models.gen,"analysis/muttama/")
plot.out<- paste0(models.gen,"analysis/muttama/plots/")
...
for (i in 1:length(data.files)){
  ...
  # GRA model average
  MA.mod<- lm(mutt_probe ~ smips + anu + usyd + awra,data = read.in)
  ...
  for (xx in 2:ncol(temp.mat)){
    cc.out[cnt]<- cccF(observed = temp.mat$mutt_probe,predicted = temp.mat[,xx])
    cnt<- cnt + 1}
  probe.ccc[i,]<-cc.out
  ...
  ylimits= c(0,200) 
  plot(read.in$date,read.in$mutt_probe,
       ylim= ylimits, 
       main = plabel,
       type= "n",
       axes=F,
...
# save outputs
write.csv(x = probe.cors, file = paste0(plot.out,"muttama_probe_model_correlations.csv"),row.names = F)
write.csv(x = gra.coeffs, file = paste0(plot.out,"muttama_probe_GRA_model_coeffs.csv"),row.names = F)
write.csv(x = probe.ccc, file = paste0(plot.out,"muttama_probe_model_concordance.csv"),row.names = F)
