...
DSM_data <- extract(x = s1, y = probe.locs, sp = 1, method = "simple")
...
plot.out<- paste0(models.gen,"analysis/bars/plots/rolling_coeffs_covs/")
...
gra.coeffs<- matrix(NA, nrow = 364, ncol = 17)
...
for (j in 25:364){
  ...
  # build up the result and model matrix
  ...
  result.dat[i,]<- read.in[nrow(read.in), ]
  }
  
  # GRA model average
  names(temp.dat)
  model.frame<- temp.dat[,c(2:6,10:21)]
...
# go through each probe
  ...
  names(template.dat)
  names(gra.coeffs)<- c("Intercept", names(template.dat)[c(3:6,10:21)])  
