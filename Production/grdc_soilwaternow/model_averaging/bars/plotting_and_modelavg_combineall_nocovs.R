...
plot.out<- paste0(models.gen,"analysis/bars/plots/combined_all/")
...
gra.coeffs<- matrix(NA, nrow = 1, ncol = 5)
...
# GRA model average
  MA.mod<- lm(bars_probe ~ smips + anu + usyd + awra,data = template.dat)
  MA.mod
  summary(MA.mod)
  ...
  # save outputs
  write.csv(x = probe.cors, file = paste0(plot.out,"bars_probe_model_correlations_combineall_nocovs.csv"),row.names = F)
  write.csv(x = gra.coeffs, file = paste0(plot.out,"bars_probe_GRA_model_coeffs_combineall_nocovs.csv"),row.names = F)
  write.csv(x = probe.ccc, file = paste0(plot.out,"bars_probe_model_concordance_combineall_nocovs.csv"),row.names = F)
